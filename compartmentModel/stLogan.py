...
class STLogan():
    ...
    def fit_tv(self,niter=10000,w=1e-2,roi=None):
        for it in tqdm(range(self.start_its[1],niter)):
            C_pred = self.model_kcm(self.t_,self.A,self.B).reshape(-1,self.im_S[0],self.im_S[1],self.im_S[2])
            loss = self.loss_fn.mseloss_tv(C_pred,self.C,
                                            k=self.model_kcm.k.reshape(-1,self.im_S[0],self.im_S[1],self.im_S[2]) * torch.tensor([[[[1.]]],[[[0.1]]]],device='cuda'), #加权
                                            w=w,roi=roi)
            loss.backward()
            self.optim_kcm.step()
            self.model_kcm.k[0,:].data.clamp_(min=0.) # DVR > 0
...
class Self2selfLoss():
    ...
    def mseloss_tv(self,y1,y2,k,w=1e-2,roi=None):
        c = k[:,1:-1,1:-1,1:-1]
        ...
        penalty3 = ( torch.abs( c - k[:,:-2,:-2,:-2] ) + torch.abs( c - k[:,2:,2:,2:]  ) +
                     ...
                     torch.abs( c - k[:,:-2,2:,:-2]  ) + torch.abs( c - k[:,2:,:-2,2:] ) +
                     torch.abs( c - k[:,:-2,:-2,2:]  ) + torch.abs( c - k[:,2:,2:,:-2] ) )
        penalty = ( penalty1 + penalty2 / 1.414 + penalty3 / 1.732 ) * w
        if roi is not None:
            return ((y1 - y2).pow(2) * roi).mean() + ( penalty * roi[1:-1,1:-1,1:-1] ).mean()
        else:
            return ((y1 - y2).pow(2)).mean() + penalty.mean()
