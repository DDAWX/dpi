...
@njit
def bprojection(lors, proj, voxelSize, image):
    dx,dy = voxelSize
...
@cuda.jit
def bprojection_cuda(lors, proj, voxelSize, image):
    i = cuda.grid(1)
    dx,dy = voxelSize
    nx,ny = image.shape[0],image.shape[1]
    nx2,ny2 = nx/2,ny/2
                ...
                for ix in range(nx):
                    ...
                    if cy1 == cy2:
                        if 0 <= cy1 < ny:
                            iy = cy1
                            weight = (1 + ky*ky) ** 0.5
                            cuda.atomic.add(image,(ix,iy),proj[i] * weight)
                    else:
                        if -1 <= cy1 < ny:
                            if cy1 >= 0:
                                iy = cy1
